...
const ADMIN_PASSWORD = "sangam@1313";
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="bg-card border border-card-border rounded-2xl shadow-lg p-8 max-w-sm w-full animate-fade-in-up">
        ...
        <button
          onClick={attempt}
          className="w-full py-2.5 bg-primary text-primary-foreground rounded-xl font-semibold hover:bg-primary/90 transition-colors text-sm"
        >
          Login
        </button>
...
function ProductForm({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
      ...
      />
      <div
        className="relative bg-card rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto animate-fade-in-up"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              <label className="flex items-center gap-2 cursor-pointer">
                <div
                  className={`w-10 h-5 rounded-full transition-colors ${form.featured ? "bg-primary" : "bg-muted"} relative`}
                  onClick={() => set("featured", !form.featured)}
                >
                  <div
                    className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all ${form.featured ? "left-5" : "left-0.5"}`}
                  />
                </div>
  ...
  const { mutate: createProduct } = useCreateProduct({
    mutation: {
      onSuccess: () => {
        ...
        qc.invalidateQueries({ queryKey: getGetStoreSummaryQueryKey() });
        setShowForm(false);
      },
    },
  });
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-background/60 text-sm">
                Sangam Kirana Store Management
              </p>
            </div>
            <button
              onClick={() => setAuthenticated(false)}
              className="ml-auto text-xs text-background/40 hover:text-background/70 border border-background/20 px-3 py-1.5 rounded-lg transition-colors"
            >
              Logout
            </button>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
        <div className="flex gap-2 mb-5">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab === "products" ? "bg-primary text-primary-foreground" : "bg-card border border-border text-muted-foreground hover:border-primary/40"}`}
          >
            Products ({products.length})
          </button>
          <button
            onClick={() => setTab("orders")}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab === "orders" ? "bg-primary text-primary-foreground" : "bg-card border border-border text-muted-foreground hover:border-primary/40"}`}
          >
          ...
          {tab === "products" && (
            <button
              ...
              className="ml-auto flex items-center gap-1.5 px-4 py-2 bg-secondary text-secondary-foreground rounded-lg text-sm font-medium hover:bg-secondary/90 transition-colors"
            >
                  ...
                  <tbody className="divide-y divide-border">
                    {products.map((p) => (
                      <tr key={p.id} className="hover:bg-muted/50">
                            ...
                            <img
                              ...
                              onError={(e) => {
                                (e.target as HTMLImageElement).src =
                                  "https://placehold.co/40x40/f5ece0/a05c2c?text=P";
                              }}
                            />
                        ...
                        <td className="px-4 py-3 text-muted-foreground text-xs hidden sm:table-cell capitalize">
                        ...
                        <td className="px-4 py-3 hidden md:table-cell">
                          <span
                            className={`text-xs px-2 py-0.5 rounded-full ${p.featured ? "bg-accent/20 text-accent-foreground" : "bg-muted text-muted-foreground"}`}
                          >
                            {p.featured ? "Yes" : "No"}
                          </span>
                        </td>
                  ...
                  <tbody className="divide-y divide-border">
                    {orders.map((o) => (
                      <tr key={o.id} className="hover:bg-muted/50">
                        ...
                        <td className="px-4 py-3 text-xs text-muted-foreground uppercase hidden md:table-cell">
                          {o.paymentMethod}
                        </td>
                        <td className="px-4 py-3">
                          <span className="text-xs bg-secondary/10 text-secondary px-2 py-0.5 rounded-full font-medium capitalize">
                            {o.status}
                          </span>
                        </td>
                      </tr>
